...
export interface SelectPromptOptions<Choices extends Readonly<Readonly<SelectChoice>[]>> extends PromptOptions {
  hint?: string
  message: string
  label: string
...
export class SelectPrompt<Choices extends Readonly<Readonly<SelectChoice>[]>> extends Prompt {
  choices: Choices
  label: string
  msg: string
  hint?: string
  ...
  render() {
    ...
    if (this.done) {
      outputText.push(`${prefix} `, color.dim(`${this.choices[this.cursor]?.label}`))
    } else {
      outputText.push(
        this.choices
          .map((choice, i) =>
            i === this.cursor
              ? `${prefix} ${color.green(shouldUseAscii() ? '>' : '●')} ${this.highlight(choice.label)} ${
                  choice.hint ? color.dim(choice.hint) : ''
                }`
              : color.dim(`${prefix} ${shouldUseAscii() ? '—' : '○'} ${choice.label} `),
          )
          .join('\n'),
      )
    }
