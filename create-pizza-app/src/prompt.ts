...
import { SelectPrompt, type SelectPromptOptions, type SelectChoice } from './prompts-select'
import { MultiSelectPrompt, type MultiSelectPromptOptions } from './prompts-multi-select'
...
const prompts = {
  ...
  select: <Choices extends readonly Readonly<SelectChoice>[]>(args: SelectPromptOptions<Choices>) =>
    toPrompt(SelectPrompt, args),
  multiselect: <Choices extends readonly Readonly<SelectChoice>[]>(args: MultiSelectPromptOptions<Choices>) =>
    toPrompt(MultiSelectPrompt, args),
}

export async function prompt<
  T extends Readonly<PromptType<any>> | Readonly<PromptType<any>[]>,
  P extends T extends Readonly<any[]> ? T[number] : T = T extends Readonly<any[]> ? T[number] : T,
>(questions: T, opts: PromptTypeOptions<P> = {}): Promise<Answers<T>> {
  ...
  let questionsArray = (Array.isArray(questions) ? questions : [questions]) as Readonly<P[]>
  let answer: Answer<P>
  let quit: any
  let name: string
  let type: P['type']
...
function toPrompt<
  T extends typeof TextPrompt | typeof ConfirmPrompt | typeof SelectPrompt<any> | typeof MultiSelectPrompt<any>,
...
type PromptType<Choices extends Readonly<SelectChoiceType[]> = Readonly<SelectChoiceType[]>> =
  | TextPromptType
  | ConfirmPromptType
...
type PromptChoices<T extends PromptType<any>> =
  T extends SelectPromptType<infer Choices> ? Choices : T extends MultiSelectPromptType<infer Choices> ? Choices : never
...
type Answers<T extends Readonly<PromptType<any>> | Readonly<PromptType<any>[]>> =
