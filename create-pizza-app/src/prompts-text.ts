...
import { color, strip, clear, lines, shouldUseAscii, type ActionKey } from './utils'
...
export class TextPrompt extends Prompt {
  ...
  render() {
    ...
    if (!this.firstRender) {
      if (this.outputError)
        this.out.write(
          cursor.down(lines(this.outputError, this.out.columns) - 1) + clear(this.outputError, this.out.columns),
        )
    ...
    if (this.error) {
      this.outputError += `  ${color.redBright((shouldUseAscii() ? '> ' : '▶ ') + this.errorMsg)}`
    }
    ...
    this.out.write(
      erase.line +
        ...
        this.outputError +
        cursor.restore +
        cursor.move(this.placeholder ? (this.rendered.length - 9) * -1 : this.cursorOffset, 0),
    )
  }
}
